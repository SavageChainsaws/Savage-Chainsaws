...
export default async function Home({
  ...
  const completed = units?.filter(u => u.status === 'Completed' || u.status === 'Ready for Pickup').length || 0
  const repairRequested = units?.filter(u => u.status === 'Repair Requested').length || 0
  const readyPickup = units?.filter(u => u.status === 'Ready for Pickup').length || 0
  const priorityCount = units?.filter(u => u.is_priority && u.status !== 'Completed').length || 0
    ...
    <main className="min-h-screen bg-black text-white p-4 sm:p-6 md:p-10">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 md:gap-4 mb-8 md:mb-10">
          ...
          <div className="bg-zinc-900 border border-orange-500/40 rounded-xl p-4 md:p-5">
