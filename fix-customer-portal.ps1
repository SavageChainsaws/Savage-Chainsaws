# Savage Chainsaws - fix customer portal + admin components auth
...
New-Item -ItemType Directory -Force -Path "app\customer" | Out-Null
New-Item -ItemType Directory -Force -Path "app\components" | Out-Null

Write-Host "Writing app\customer\page.tsx..." -ForegroundColor Cyan
$customer_page_tsx = @'
'use client'
...
'@
Set-Content -Path "app\customer\page.tsx" -Value $customer_page_tsx -Encoding UTF8

Write-Host "Writing app\components\AdminLogout.tsx..." -ForegroundColor Cyan
$AdminLogout_tsx = @'
'use client'
...
'@
Set-Content -Path "app\components\AdminLogout.tsx" -Value $AdminLogout_tsx -Encoding UTF8

Write-Host "Writing app\components\CheckInForm.tsx..." -ForegroundColor Cyan
$CheckInForm_tsx = @'
'use client'
...
'@
Set-Content -Path "app\components\CheckInForm.tsx" -Value $CheckInForm_tsx -Encoding UTF8

Write-Host "Writing app\components\DeleteUnitButton.tsx..." -ForegroundColor Cyan
$DeleteUnitButton_tsx = @'
'use client'
...
'@
Set-Content -Path "app\components\DeleteUnitButton.tsx" -Value $DeleteUnitButton_tsx -Encoding UTF8

Write-Host ""
Write-Host "Done writing files." -ForegroundColor Green
Write-Host ""
...
git add .
git commit -m "Fix customer portal and admin components to use shared cookie-based auth client"
git push
Write-Host ""
Write-Host "All done. Vercel will auto-deploy from this push." -ForegroundColor Green